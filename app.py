...
# ─────────────────────────────────────────────────────────────
#  SPOTIFY PLAYLIST  — scrape track names, search on YouTube
#  No Spotify API key needed — uses open.spotify.com embed data
# ─────────────────────────────────────────────────────────────
...
def api_spotify_playlist():
    ...
    # Extract playlist ID
    m = _re.search(r'spotify\.com/playlist/([a-zA-Z0-9]+)', url)
    if not m:
        return jsonify({'error': 'Invalid Spotify playlist URL'}), 400
    playlist_id = m.group(1)

    # Scrape Spotify embed page (no auth needed — public playlists only)
    try:
        ...
        if not r.ok:
            return jsonify({'error': f'Spotify returned {r.status_code}'}), 502
        html_text = r.text

        # Extract __NEXT_DATA__ JSON which contains track list
        next_data_match = _re.search(r'<script id="__NEXT_DATA__" type="application/json">(.*?)</script>', html_text, _re.DOTALL)
        tracks = []
        if next_data_match:
            try:
                ...
                items = (data.get('props',{}).get('pageProps',{})
                             .get('state',{}).get('data',{})
                             .get('entity',{}).get('trackList',[]))
                for item in items:
                    title = item.get('title','').strip()
                    subtitle = item.get('subtitle','').strip()  # artist name
                    if title:
                        tracks.append((title, subtitle))
            except Exception:
                pass

        if not tracks:
            # Fallback: regex scrape track titles from HTML
            raw_titles = _re.findall(r'"name"\s*:\s*"([^"]{2,80})"', html_text)
            # Filter out playlist/genre names (usually short, repeated)
            seen = set()
            for t in raw_titles:
        ...
        return jsonify({'error': f'Spotify fetch failed: {e}'}), 502
    ...
    def search_track(title, artist):
        ...
        # Try cache first
        cached, _ = cache_get(q)
        if cached and cached[0]:
            return cached[0]
        # Try YouTube API keys
        if yt_keys:
            result = _search_youtube_key(q, yt_keys[0])
            if result:
                cache_set(q, result, 'youtube')
                return result[0]
        # Fallback to no-key search
        result = _search_youtube_nokey(q)
        if result:
            cache_set(q, result, 'nokey')
            return result[0]
        return None
    ...
    for job, (title, artist) in zip(jobs, tracks[:50]):
        ...
        if hit:
            hit = dict(hit)
            hit['qid'] = hit['id'] + '_' + str(int(_time.time()*1000)) + '_' + str(len(songs))
            songs.append(hit)

    if not songs:
        return jsonify({'error': 'No YouTube matches found'}), 404
